...
from src.challenge1.observers import EmailNotifier, SlackNotifier, LoggingObserver
...
def test_attach_observer(issue, email_observer):
    issue.attach(email_observer)
    assert email_observer in issue._observers


def test_detach_observer(issue, email_observer):
    issue.attach(email_observer)
    issue.detach(email_observer)
    assert email_observer not in issue._observers


def test_multiple_observers(issue, email_observer, slack_observer, logging_observer, capsys):
...
def test_status_update_notification(issue, email_observer, capsys):
    issue.attach(email_observer)
    issue.update_status(IssueStatus.IN_PROGRESS)
...
def test_assignment_notification(issue, slack_observer, capsys):
    issue.attach(slack_observer)
...
def test_no_notification_on_same_value(issue, email_observer, capsys):
    issue.attach(email_observer)
